...
\begin{tikzpicture}[scale=0.65]
  ...
  \draw[->] ( -1*\X, 0*\Y) node[anchor=east, align=center]{Rest\\of the\\system} -- ( -5+0*\X, 0*\Y);
  \draw[->] ( -1*\X, -1*\Y) -- ( -5+0*\X, 0*\Y);

  \draw[<-] ( 5*\X, 1*\Y) -- ( 5+4*\X, 0*\Y);
  \draw[<-] ( 5*\X, 0*\Y) node[anchor=west, align=center]{Rest\\of the\\system}-- ( 5+4*\X, 0*\Y);
  \draw[<-] ( 5*\X, -1*\Y) -- ( 5+4*\X, 0*\Y);
  ...
  \draw[->](5+2*\X,  2*\Y) -- node[sloped, above]{$d\,\GO{\pmb{a'\,a}}$} (-5+4*\X, 0*\Y); %% E->F
  \draw[->](5+2*\X,  0*\Y) -- node[sloped, above]{$\GO{\pmb{a'}}\,b\,\GO{\pmb{a}}\,c$} (-5+4*\X, 0*\Y); %% D->F
