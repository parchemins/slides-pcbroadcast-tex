...
\begin{tikzpicture}
  ...
  \draw [->] (0*\X, 0.5*\Y) node[left]{\faImage} -- (1*\X, 1.5*\Y); %% A B
  \draw [->] (0*\X, 0.5*\Y) -- (2*\X, 4.5*\Y); %% A C

  \draw [->] (1*\X, 2*\Y) node[above right]{\faCommentingO} -- (2*\X, 3.5*\Y); %% B C
  \draw [->] (1*\X, 2*\Y) -- (0*\X, 3.5*\Y); %% B A
